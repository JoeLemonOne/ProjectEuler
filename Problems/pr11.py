"""
В таблице S 20×20 четыре числа на одной диагонали выделены красным.
...
S = [[8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8],
...
for i in range(16):
    for j in range(16):
        product = S[i][j] * S[i + 1][j + 1] * S[i + 2][j + 2] * S[i + 3][j + 3]
        # print(product)
        if product > max_product:
            max_product = product
        product = S[i + 3][j] * S[i + 2][j + 1] * S[i + 1][j + 2] * S[i][j + 3]
        if product > max_product:
            max_product = product
